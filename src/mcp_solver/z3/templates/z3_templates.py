...
try:
    from z3 import (
        Int, Ints, And, Or, Not, Implies, 
        ForAll, Exists, PbEq, PbLe, PbGe,
        BoolRef, ArrayRef, ExprRef
    )
except ImportError:
...
def array_is_sorted(arr: ArrayRef, size: Union[int, ExprRef], strict: bool = False) -> BoolRef:
    """
...
def function_is_injective(func: ArrayRef, domain_size: Union[int, ExprRef], range_size: Union[int, ExprRef] = None) -> BoolRef:
    """
...
def function_is_surjective(func: ArrayRef, domain_size: Union[int, ExprRef], range_size: Union[int, ExprRef]) -> BoolRef:
    """
