...
def smallest_subset_with_property(items: List[T], 
                                 property_check_func: Callable[[List[T]], bool], 
                                 min_size: int = 1, 
                                 max_size: Optional[int] = None) -> Optional[List[T]]:
    """
...
def _minimize_subset(subset: List[T], property_check_func: Callable[[List[T]], bool]) -> List[T]:
