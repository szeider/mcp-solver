...
def execute_pysat_code(code_string: str, timeout: float = 4.0) -> Dict[str, Any]:
    ...
    try:
        ...
        from pysat.solvers import (
            ...
            Lingeling, MapleCM, MinisatGH,
            Minisat22, Minicard, Mergesat3
        )
    ...
    for line in code_lines:
        ...
        if (line.strip().startswith('from pysat import') or 
            line.strip().startswith('import pysat') or
            line.strip().startswith('from pysat.formula import') or
            line.strip().startswith('from pysat.solvers import') or
            line.strip().startswith('from pysat.examples import') or
            line.strip().startswith('from pysat.card import')):
            continue
        else:
    ...
    try:
        ...
        if memory_tracking:
            ...
            if final_memory - initial_memory > 50:  # More than 50MB growth
                print("WARNING: Significant memory growth detected. Possible memory leak.")
...
def cardinality_constraints_test():
    ...
    try:
        ...
        print(f"Generated {len(cnf.clauses)} clauses for 'at most 2 of 5 variables' constraint")
        ...
        for num_true in range(6):  # 0 to 5 true variables
            ...
            expected = num_true <= 2  # Should be satisfiable only when ≤ 2 variables are true
            
            print(f"  {num_true} true variables: {'Satisfiable' if is_sat else 'Unsatisfiable'} " +
        ...
        for num_true in range(6):  # 0 to 5 true variables
            ...
            is_sat = solver.solve(assumptions=assumptions)
            expected = num_true == 2  # Should be satisfiable only when exactly 2 variables are true
...
def maxsat_with_constraints_test():
    ...
    try:
        ...
        at_most_3 = CardEnc.atmost([1, 2, 3, 4, 5], bound=3, encoding=EncType.seqcounter)
        
        # Add each clause from the CNF to the WCNF as a hard constraint
        for clause in at_most_3.clauses:
            wcnf.append(clause)
            
        print(f"Created WCNF with {len(wcnf.hard)} hard clauses and {len(wcnf.soft)} soft clauses")
        ...
        with RC2(wcnf) as rc2:
            ...
            if model:
                ...
                total_value = sum(
                    [5, 3, 8, 2, 7][task-1] for task in selected_tasks
                )
                ...
                print(f"  Percentage of maximum: {round(total_value / max_possible * 100, 2)}%")
