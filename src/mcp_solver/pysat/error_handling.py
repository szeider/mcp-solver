...
EXCEPTION_MESSAGES = {
    "TypeError": {
        "object of type 'NoneType' has no len()": 
            "Formula appears to be empty or not properly initialized",
        "append_formula() missing 1 required positional argument: 'formula'":
            "No formula was provided to the solver",
        "vars() got an unexpected keyword argument": 
            "Incorrect variable definition. Variables must be positive integers",
        "'int' object is not iterable":
            "A clause must be a list of integers, not a single integer",
    },
    "ValueError": {
        "variable identifier should be a non-zero integer":
            "Variable IDs must be non-zero integers. Check your variable mapping",
        "literal should be a non-zero integer":
            "Clause literals must be non-zero integers. Zero is not allowed in clauses",
        "unexpected literal value":
            "Invalid literal value in clause. Literals must be integers",
    },
    "RuntimeError": {
        "solver is not initialized": 
            "Solver was not properly initialized before use",
        "solver is in an invalid state":
            "Solver has been corrupted or used after being deleted",
    },
    "AttributeError": {
        "'NoneType' object has no attribute": 
            "Attempted to use a solver or formula that doesn't exist",
        "object has no attribute 'append_formula'":
            "The solver object doesn't support append_formula. Make sure you're using a compatible solver",
    }
}
