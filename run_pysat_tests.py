...
# Configuration
PROMPT_FILE = "instructions_prompt_pysat_lite.md"  # PySAT lite prompt file
PYSAT_PROBLEMS_DIR = "problems/pysat"
...
def run_test(problem_file, verbose=False, timeout=DEFAULT_TIMEOUT):
    ...
    cmd = f"uv run test-client-pysat --query {abs_problem_path}"
    if verbose:
...
def main():
    ...
    if args.problem:
        ...
        problem_files = glob.glob(f"{PYSAT_PROBLEMS_DIR}/{args.problem}.md")
        if not problem_files:
            print(f"Error: Problem file '{args.problem}.md' not found in {PYSAT_PROBLEMS_DIR}")
            sys.exit(1)
    else:
        # Test all problems
    ...
    print(f"SUMMARY: PySAT Tests ({current_time})")
    print(f"{'='*60}")
